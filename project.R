...
ggplot(df, aes(m_form, freq))+geom_bar(stat = 'identity', fill = "tomato4")+coord_flip() +my_theme+labs(y = "Frequency", x = " Forms of meditation", title = "Rajat RKMVERI")
...
ggplot(c1, aes(m_when, count, fill = age)) + geom_bar(stat = 'identity', position = "dodge") +
...
ggplot(df, aes(m_reason, freq))+geom_bar(stat = 'identity', fill = "tomato2")+coord_flip() +my_theme
